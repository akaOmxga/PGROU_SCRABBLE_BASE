...
export class Joueur {
  ...
  async completerLettres(pioche) {
    console.log("Compléter lettres - lettres actuelles:", this.lettres.length);
    while (this.lettres.length < 7) {
      const lettre = await pioche.piocherLettre();
      console.log("Lettre piochée:", lettre);
      if (lettre) {
        this.ajouterLettre(lettre);
      } else {
        break; // Plus de lettres disponibles
      }
    }
